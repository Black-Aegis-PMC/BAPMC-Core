...
if (!local _source) exitWith {
    diag_log format ["[lxRF_fnc_RC40_recon] %1: failed due to using non-local object",_source] ;
} ;
...
if (_source isKindOf "CAManBase") exitWith {
    ...
    if (behaviour _source == "SAFE" or behaviour _source == "CARELESS") exitWith {
        diag_log format ["[lxRF_fnc_RC40_recon] %1: failed due for being behaviour either SAFE or CARELSS",_source] ;
    } ;
    ...
    diag_log format ["[lxRF_fnc_RC40_recon] %1: fire started",_source] ;

    if !("1Rnd_RC40_shell_RF" in flatten _magazines) exitWith {
        diag_log "[lxRF_fnc_RC40_recon] Unit fire ended: the unit has no compatible magazine" ;
    } ;    // exit if the soldier doesn't have an RC40 HE

    private _muzzles = _source weaponsInfo [""] ;
    private _compatibleMuzzle = _muzzles findIf {"1Rnd_RC40_shell_RF" in compatibleMagazines [_x#2,_x#3]} ;
    if (_compatibleMuzzle == -1) exitWith {
        diag_log format ["[lxRF_fnc_RC40_recon] %1: fire ended: the unit has no compatible weapon",_source] ;
    ...
    [_source,_dummyTarget,_target,_muzzles#_compatibleMuzzle#3,call {
        ...
        diag_log format ["[lxRF_fnc_RC40_recon] %1: fire progress: aim started",_source] ;
        _source reload [_weapon, "1Rnd_RC40_shell_RF"] ;
        ...
        if (time - _timeNow > 5) exitWith {
            ...
            diag_log format ["[lxRF_fnc_RC40_recon] %1: fire ended: aim failed, timeout and exit",_source] ;
        } ;
        ...
        [_source,"lxRF_RC40_deploy_fired",{
            ...
            private _target = _source getVariable ["lxRF_fnc_RC40_recon_target",objNull] ;
            //[_rc40,_target] call lxRF_fnc_RC40_recon ;
            diag_log format ["[lxRF_fnc_RC40_recon] %1: fire progress: RC40 successfully deployed",_source] ;
			[_rc40,_source]spawn DDT_fnc_RC40recon;
        }] call BIS_fnc_addScriptedEventHandler;
        _source setVariable ["lxRF_fnc_RC40_recon_target",_target] ;
        sleep 0.7 ;
        _source enableAI"MOVE" ;
        deleteVehicle _dummyTarget ;
        _source doWatch objNull ;
        diag_log format ["[lxRF_fnc_RC40_recon] %1: fire ended: function successfully ended",_source] ;
    } ;
} ;