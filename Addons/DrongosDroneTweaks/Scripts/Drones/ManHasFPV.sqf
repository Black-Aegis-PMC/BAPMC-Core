...
if([(magazines _man),ddtClassesFPV]call DDT_fnc_InArray)exitWith{TRUE};
if([(items _man),ddtClassesFPV]call DDT_fnc_InArray)exitWith{TRUE};
private _pack=backpack _man;
if(_pack=="")exitWith{FALSE};
if(_pack call DDT_fnc_BackpackIsFPV)exitWith{TRUE};
FALSE