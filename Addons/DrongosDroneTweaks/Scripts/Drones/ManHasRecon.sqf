...
if([(magazines _man),ddtClassesRecon]call DDT_fnc_InArray)exitWith{TRUE};
if([(items _man),ddtClassesRecon]call DDT_fnc_InArray)exitWith{TRUE};
private _pack=backpack _man;
if(_pack=="")exitWith{FALSE};
if(_pack call DDT_fnc_BackpackIsRecon)exitWith{TRUE};
FALSE